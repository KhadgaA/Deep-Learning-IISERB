...
def extract_frames(video_path, frames_dir, overwrite=False, start=-1, end=-1, every=1):
    ...
    while frame < end:  # lets loop through the frames until the end
        ...
        if frame % every == 0:  # if this is a frame we want to write out based on the 'every' argument
            ...
            save_path = os.path.join(frames_dir, "{:04d}.png".format(frame))  # create the save path
...
if __name__ == '__main__':
    ...
    if args.dataset == 'all':
        for dataset in DATASET_PATHS.keys():
            ...
            for video in tqdm(os.listdir(videos_path),desc=dataset):
                image_folder = video.split('.')[0]
                if os.path.exists(os.path.join(images_path, image_folder)):
                    print(f'file already exists. Skipping video : {image_folder}')
                    continue
                else:
                    # extract_frames(join(videos_path, video),
                    #                join(images_path, image_folder))
                    # test it
                    video_to_frames(video_path=os.path.join(videos_path, video),
                                    frames_dir=os.path.join(images_path, image_folder), overwrite=True, every=1,
                                    chunk_size=20)
    else:
