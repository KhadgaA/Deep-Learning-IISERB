import torch
from utils import data_prefetcher_two, cal_fam, setup_seed, calRes
from pretrainedmodels import xception
import datasets_profiles as dp
...
parser.add_argument('--logbatch', default=800, type=int)
parser.add_argument('--savebatch', default=800, type=int)
...
def Log(log):
    ...
    if os.path.exists(path):
        ...
        os.mkdir(path)
    f = open(path+upper+"_"+modelname+".log", "a")
    f.write(log+"\n")
    f.close()
...
if __name__ == "__main__":
    ...
    for e in range(args.epochs):
        ...
        pbar = tqdm(total=args.max_batch)
        while data is not None and batchind < args.max_batch:
            ...
            batchind += 1
            #pbar.set_description("Train %06d loss:%.5f avgloss:%.5f lr:%.6f time:%.4f" % (batchind, loss, sumloss/sumcnt, optim.param_groups[0]["lr"], time.time()-stime))
            ...
            if batchind % args.logbatch == 0:
                ...
                plt.subplot(121)
                plt.imshow(inputs[0].detach().to(torch.device('cpu')).squeeze().permute(1,2,0))
                plt.subplot(122)
                plt.imshow(inputs_fam[0].detach().to(torch.device('cpu')).squeeze().permute(1, 2, 0))
                plt.savefig(f'testfig_{e}_{batchind}')
